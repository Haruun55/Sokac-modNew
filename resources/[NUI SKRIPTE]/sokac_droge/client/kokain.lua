				...
				if not preraduje then
					...
					ESX.TriggerServerCallback('esx_illegal:have', function (count)
						if count >= 5 then
							ESX.TriggerServerCallback('esx_illegal:have', function (count)
								if count >= 1 then
									ESX.ShowNotification('Preradujes kokain')
								else
									preraduje = false
						ESX.ShowNotification('Nemas dovoljno glukoze')
								end
							end, 'glukoza')
						else
							preraduje = false
							ESX.ShowNotification('Nemas dovoljno listova kokaina')
						end
					end, 'list_koke')
				end
			end
	...
	options = {
		{
			...
			action = function (entity)
				...
				ESX.ShowNotification('Ne mozes da nosis vise')
							bere = false
						end
