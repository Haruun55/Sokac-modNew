...
function StartTheoryTest(success)
	CurrentTest = 'theory'
	...
	if success then
		TriggerServerEvent('angelsrp_autoskola:addLicense', 'dmv')
		ESX.ShowNotification(_U('passed_test'))
	else
		ESX.ShowNotification(_U('failed_test'))
	end
	TriggerServerEvent('angelsrp_autoskola:pay', Config.Prices['dmv'])
end

local autoskola= {
	`a_m_y_cyclist_01`,
}
exports['qtarget']:AddTargetModel(autoskola, {
	options = {
		{
			event = "autoskola-bog",
			icon = "fas fa-car",
...
RegisterNetEvent("autoskola-bog")
AddEventHandler("autoskola-bog", function()
	StartTheoryTest(success)
end)
...
function StartDriveTest(type)
	...
	TriggerServerEvent('angelsrp_autoskola:pay', Config.Prices[type])
end

function StopDriveTest(success)
	if success then
		TriggerServerEvent('angelsrp_autoskola:addLicense', CurrentTestType)
		ESX.ShowNotification(_U('passed_test'))
	else
		ESX.ShowNotification(_U('failed_test'))
	end
...
AddEventHandler('angelsrp_autoskola:hasEnteredMarker', function(zone)
...
AddEventHandler('angelsrp_autoskola:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)

RegisterNetEvent('angelsrp_autoskola:loadLicenses')
AddEventHandler('angelsrp_autoskola:loadLicenses', function(licenses)
	Licenses = licenses
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('angelsrp_autoskola:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('angelsrp_autoskola:hasExitedMarker', LastZone)
		end
	end
end)
