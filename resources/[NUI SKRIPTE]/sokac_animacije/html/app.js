    ...
    switch (v.action) {
        ...
        case 'LoadEmotes':
            ...
            $(`#${v.emotename}`).click(function() {
                emote = this.id
                $.post('https://sokac_animacije/executeemote', JSON.stringify({
                    emote: emote,
                    type: 'emote',
                }));
            });
...
$(document).keyup((e) => {
    if (e.key === "Escape") {
        setTimeout(() => {
            $.post('https://sokac_animacije/exit', JSON.stringify({}));
        }, 300);
    }
});
...
window.addEventListener('load', () => {
    $.post('https://sokac_animacije/callAnimations', JSON.stringify({})); 
});