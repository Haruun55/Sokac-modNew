    ...
    while true do
        ...
        if hasjob then
            if not DoesBlipExist(startBlip) then
                startBlip = addBlip(Config.JobCenter, 545, 0, Prevod['start_job'])
            end
        else
...
AddEventHandler('esx_ets:start', function(data, jobid)
    ...
    local destinationBlip, truckBlip, trailerBlip = addBlip(data.arrive, 38, 3, Prevod['destination'])
    while true do
        ...
        if not IsPedInVehicle(PlayerPedId(), truck, false) then
            text(Prevod['get_to_truck'], 250)
            if not DoesBlipExist(truckBlip) then
                truckBlip = addBlip(GetEntityCoords(truck), 477, 5, Prevod['truck']) 
            ...
            if not IsVehicleAttachedToTrailer(truck, trailer) then
                text(Prevod['get_to_trailer'], 250)
                if not DoesBlipExist(trailerBlip) then 
                    trailerBlip = addBlip(GetEntityCoords(trailer), 479, 5, Prevod['trailer']) 
                ...
                if not DoesBlipExist(destinationBlip) then
                    destinationBlip = addBlip(data.arrive, 38, 3, Prevod['destination'])
        ...
        if distance <= 45.0 and IsPedInVehicle(PlayerPedId(), truck, false) and IsVehicleAttachedToTrailer(truck, trailer) then
            ...
            if IsPedInVehicle(PlayerPedId(), truck, false) and IsVehicleAttachedToTrailer(truck, trailer) then
                text(Prevod['drive_destination'], 250)
            end
        end
        Wait(sleep)
    end
    ...
    while true do
        ...
        if not IsPedInVehicle(PlayerPedId(), truck, false) and distance >= 10.0 then
            ...
            if DoesBlipExist(truckBlip) then RemoveBlip(truckBlip) end
            if not DoesBlipExist(destinationBlip) then
                destinationBlip = addBlip(data.start[1], 38, 3, Prevod['destination'])
                SetBlipRoute(destinationBlip, true)
            end
            if distance <= 30.0 then
                ...
                if distance <= 10.0 then
                    ...
                    text(Prevod['park_truck'])
                end
            else
                text(Prevod['drive_back'], 250)
            end
        end
        Wait(sleep)
    end
