...
AddEventHandler('esx_ets:startJob', function(id)
    ...
    if hasJob then
        if not Config.Jobs[id]['taken'] then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, Prevod['somebody_doing'])
        end
    else
        TriggerClientEvent('esx:showNotification', xPlayer.source, Prevod['not_job'])
    end
end)
...
AddEventHandler('esx_ets:jobFinished', function(id, damages)
    ...
    if Config.Jobs[id]['taken'] then
        ...
        if price >= 0 then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, (Prevod['reward']):format(price))
        else
            xPlayer.removeMoney(price)
            TriggerClientEvent('esx:showNotification', xPlayer.source, (Prevod['paid_damages']):format(price))
        end
