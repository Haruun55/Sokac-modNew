...
$(function () {
    ...
    function CloseMenu(save) {
        $.post('http://sokac_butik/escape', JSON.stringify({save:save}));
    }

    $(document).on('contextmenu', function() {
        $.post('http://sokac_butik/togglecursor', JSON.stringify({}));
    })
    ...
    $('#savedright').on('click', function () {
        ...
        $.post('http://sokac_butik/loadsaved', JSON.stringify({
            "number": input.val()
        }));
    })
    ...
    $('#savemenu').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('http://sokac_butik/saveescape', JSON.stringify($(this).val()))
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('http://sokac_butik/customskin', JSON.stringify($(this).val()));
        }
    })
    ...
    function inputChange(ele,inputType) {
        ...
        if (currentMenu.is($('#clothesmenu')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            $.post('http://sokac_butik/updateclothes', JSON.stringify({
                "name": nameId,
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('http://sokac_butik/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('http://sokac_butik/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('http://sokac_butik/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('http://sokac_butik/switchcam', JSON.stringify({name: 'cam'}))
    })


    $('.tog_hat').on('click', function() {
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('http://sokac_butik/toggleclothes', JSON.stringify({name: "masks"}))
    })

    $('#reset').on('click', function() {
        $.post('http://sokac_butik/resetped', JSON.stringify({}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            $.post('http://sokac_butik/rotate', JSON.stringify({key: _key}))
        }
    }, 50))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('http://sokac_butik/savehaircolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value')
                }));
            }
            else {
                $.post('http://sokac_butik/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),
