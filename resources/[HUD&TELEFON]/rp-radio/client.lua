...
RegisterKeyMapping("opencloseradio2", "Otvori/Zatvori radio", "MOUSE_BUTTON", "MOUSE_MIDDLE")
RegisterCommand("opencloseradio2", function(src, args, raw)
	...
	ESX.TriggerServerCallback("radio:ima", function(ima) 
		if ima then 
			ima = true 

			TriggerEvent("radio:funkcija")
			...
			ima = false
			exports['mythic_notify']:SendAlert('error', 'Nemas radio', 5000)
			ClearInterval('radio')
			Radio:Toggle(false)
			Radio.On = false
			Radio:Remove()
...
RegisterNetEvent("radio:jebacina")
AddEventHandler("radio:jebacina", function()

	ClearInterval('radio')
end)
AddEventHandler("radio:funkcija", function()
	-- Add Labels
