...
RegisterKeyMapping('mobilni', 'Telefon', 'KEYBOARD', 'F1')
RegisterCommand('mobilni', function()
  TooglePhone()
end)
CreateThread(function()
  while true do
...
AddEventHandler("gcPhone:TgiannSes", function(phoneId)
  if GetPlayerServerId(PlayerId()) == tonumber(phoneId) then
    ESX.TriggerServerCallback('crew-phone:phone-check', function(durum)
      if durum ~= nil then
        ESX.ShowNotification('Zvoni vam telefon!')
      end
    end)
  end
end)
    ...
    ESX.TriggerServerCallback('crew-phone:phone-check', function(durum)
      if durum ~= nil then
        ...
        if ShowNumberNotification == true then
          if message.transmitter == "policija" or message.transmitter == "hitna" then
            text = message.transmitter.. _U('new_message_hotline')
          else
            ESX.ShowNotification('' .. message.transmitter .. ' vam je posalo poruku!')
          end
          for _,contact in pairs(contacts) do
            if contact.number == message.transmitter then
              ESX.ShowNotification('' .. contact.display .. ' vam je posalo poruku!')
              break
            end
          end
        end
