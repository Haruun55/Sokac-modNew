...
RegisterNetEvent("bSvastara:SviAdminiShow")
AddEventHandler("bSvastara:SviAdminiShow", function(showAdmini)
    SviAdminiShow = showAdmini
end)
...
AddEventHandler('onClientResourceStop', function (resourceName)
	...
	for k,v in pairs(SviAdminiTagovi) do
		RemoveMpGamerTag(v)
	end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		for i = 1, #SviAdminiShow do
			local textTag = ""
			local bojaTag = 0

			local player = GetPlayerFromServerId(SviAdminiShow[i].src)
			
			if SviAdminiShow[i].grupa == "admin" then
				textTag =  SviAdminiShow[i].ime .. " | ADMIN "
				bojaTag = 137
			elseif SviAdminiShow[i].grupa == "superadmin" then
				textTag =  SviAdminiShow[i].ime .. " | SUPER ADMIN "
				bojaTag = 142
			elseif SviAdminiShow[i].grupa == "owner" then
				textTag =  SviAdminiShow[i].ime .. " | VLASNIK "
				bojaTag = 027
			elseif SviAdminiShow[i].grupa == "user" then
				textTag =  SviAdminiShow[i].ime .. " | HELPER "
				bojaTag = 0
			end


			if not SviAdminiShow[i].speca and player ~= -1 and NetworkIsPlayerActive(player) then

				SviAdminiTagovi[ SviAdminiShow[i].src ] = CreateFakeMpGamerTag(GetPlayerPed(player), textTag, false, true, '', 0)

				SetMpGamerTagVisibility(SviAdminiTagovi[ SviAdminiShow[i].src ], 0, true)
				SetMpGamerTagColour( SviAdminiTagovi[ SviAdminiShow[i].src ], 0, bojaTag)
...
--[[ RegisterNetEvent("bAdmin:ResetujTagove")
AddEventHandler("bAdmin:ResetujTagove", function()
...
CreateThread(function()
	while true do
		...
		SetDiscordAppId(1020983208185188372)
		SetDiscordRichPresenceAsset('otac')
		SetDiscordRichPresenceAssetText("Sokac mod uskoro !")
		SetRichPresence("ID: " .. GetPlayerServerId(PlayerId()) .. " | " .. GlobalState["BrojIgraca"] .. "/128")
	end
  end)
