...
window.addEventListener('message', function (event) {
    ...
    if (data.action == "open") {
      $(".wrapper").show()
      $(".OrgTitle").html(`Poslovni papiri firme: ${data.jobLabel}`)
      ...
      resp.forEach(function(item){
        $(".plate_div .tbl-content tbody").append(`
          <tr plataID = ${item.grade}>
            ...
            <td><div class = "in_tableButton promijeni_platu">SPREMI PLACU</div></td>
          </tr>
        `)
      ...
      resp.forEach(function(item){
        $(".clanovi_div .tbl-content tbody").append(`
          <tr personID = ${item.identifier} grade = ${item.grade_number}>
            ...
            <td><div class = "in_tableButton otpusti_clana" id = "otpusti_clana"><i class="fas fa-times-circle"></i> OTPUSTI</div></td>
          </tr>
        `)
        ...
        resp.forEach(function(item){
          $(".permisije_div .tbl-content tbody").append(`
            <tr ovajRank = ${item.grade} >
              <td>${item.label}</td>
              <td vrsta = "uplacivanje">${item.uplacivanje ? "Da" : "Ne"}</td>
              <td vrsta = "podizanje">${item.podizanje ? "Da" : "Ne"}</td>
              <td vrsta = "rankovi">${item.rankovi ? "Da" : "Ne"}</td>
              <td vrsta = "clanovi">${item.clanovi ? "Da" : "Ne"}</td>
              <td vrsta = "zaposljavanje">${item.zaposljavanje ? "Da" : "Ne"}</td>
              <td vrsta = "permisije">${item.permisije ? "Da" : "Ne"}</td>
              <td vrsta = "plate">${item.plate ? "Da" : "Ne"}</td>
            </tr>
          `)
        })
