...
StvoriVozilo = function(vozilo)
    ...
    ESX.Game.SpawnVehicle(vozilo, Config.Mafije[PlayerData.job.name]["Vehicles"][1], GetEntityHeading(ped), function(veh)
        ...
        SetVehicleFuelLevel(veh, 75.0)
        SetVehicleOilLevel(veh, 75.0) -- eto da ima full ulje, ne znam jeli ovo ista znaci ??
		SetVehicleNumberPlateText(veh, "LSHG"..tostring(math.random(1000, 9999)))
        ...
        if PlayerData.job.name == 'gsf' then
            ...
            elseif PlayerData.job.name == 'lcn' then -- Marabunta
                    SetVehicleColours(veh, 83, 83)
            elseif PlayerData.job.name == 'peaky' then -- Restoran Paleto
                    SetVehicleColours(veh, 35, 35)
            elseif PlayerData.job.name == 'camorra' then -- Vinarija
                    SetVehicleColours(veh, 12, 12)
            elseif PlayerData.job.name == 'ludisrbi' then -- Talijano restorano
                    SetVehicleColours(veh, 3, 3)
		  elseif PlayerData.job.name == 'automafija' then -- Celo org 
                    SetVehicleColours(veh, 12, 12)
            elseif PlayerData.job.name == 'vagos' then
                    SetVehicleColours(veh, 42, 42)
...
TriggerEvent("rc_alert:startAlert", 5000, "NOTIFIKACIJA", "Osoba nije u vozilu!")
	end
end)
...
AddEventHandler('Pretrazi', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if PlayerData.job and Config.Mafije[PlayerData.job.name] then
        if closestPlayer ~= -1 and closestDistance <= 3.0 then
		exports.ox_inventory:openInventory('player', GetPlayerServerId(player))
             ...
             TriggerEvent("rc_alert:startAlert", 5000, "NOTIFIKACIJA", "Nisi u organizaciji!")
            end
end)
AddEventHandler('vezi', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
        if PlayerData.job and Config.Mafije[PlayerData.job.name] then
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
	TriggerServerEvent('esxbalkan_mafije:vezivanje', GetPlayerServerId(closestPlayer))
...
AddEventHandler('staviuvozilo', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
        if PlayerData.job and Config.Mafije[PlayerData.job.name] then
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		TriggerServerEvent('esxbalkan_mafije:staviUVozilo', GetPlayerServerId(closestPlayer))
...
AddEventHandler('izvadiizvozila', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
        if PlayerData.job and Config.Mafije[PlayerData.job.name] then
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
	TriggerServerEvent('esxbalkan_mafije:staviVanVozila', GetPlayerServerId(closestPlayer))
...
exports.qtarget:Player({
   ...
   RegisterNetEvent("CarryPeople:sync")
    AddEventHandler("CarryPeople:sync", function()
